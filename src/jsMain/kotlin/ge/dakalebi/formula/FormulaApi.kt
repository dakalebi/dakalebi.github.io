 ...
 * be a static bundle with no backend.
 *
 * We deliberately send no custom headers. The original server-side client set
 * User-Agent / Origin / Referer, all of which are forbidden to scripts, and any
 * non-safelisted header would force a preflight the API does not advertise
...
object FormulaApi {
    ...
    private suspend fun getText(path: String): String {
        val response = window.fetch("$BASE/$path").await()
        if (!response.ok) {
            throw FormulaApiException("Formula API $path failed: ${response.status}")
        }
