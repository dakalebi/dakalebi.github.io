...
/**
 * Per-device preferences in `localStorage`, with the play intent in `sessionStorage` because
 * it should not outlive the tab.
 *
 * Same keys as the root app's `BrowserPreferencesRepository`, deliberately: the two apps are
 * served from one origin, so a viewer who moves between `/` and `/preview` keeps their
 * language, autoplay and quality choices.
 *
 * Storage only — no Compose state. The observable half is
 * [ge.dakalebi.presentation.PreferencesStore], which reads through this.
 */
class WasmPreferencesRepository : PreferencesRepository {
    ...
    /**
     * Defaults to on, which is what iPhone and iPad did before there was a choice — so the
     * switch changes nothing until someone touches it. Hence "not off" rather than "is on":
     * an absent key has to mean the default.
     */
    ...
    /**
     * Whether this episode was left playing or paused, remembered for the session. Without
     * it, navigating next/back always resumes playback even when the viewer had deliberately
     * paused.
     */
    override fun playIntent(episodeId: String): String? = runCatching {
        sessionStorageGet(INTENT_PREFIX + episodeId)
    }.orWarn("read session $episodeId")?.takeIf { it == "playing" || it == "paused" }

    override fun setPlayIntent(episodeId: String, intent: String) {
        runCatching { sessionStorageSet(INTENT_PREFIX + episodeId, intent) }
    ...
    private fun read(key: String): String? =
        runCatching { localStorageGet(key) }.orWarn("read $key")

    private fun readBool(key: String): Boolean = read(key) == "1"

    private fun write(key: String, value: String) {
        runCatching { localStorageSet(key, value) }.orWarn("write $key")
    }

    private fun writeBool(key: String, value: Boolean) = write(key, if (value) "1" else "0")

    private fun remove(key: String) {
        runCatching { localStorageRemove(key) }.orWarn("remove $key")
    }

    /**
     * Storage throws outright in Safari private mode and wherever the user has blocked site
     * data. Preferences are not worth failing over, but "my volume never sticks" is otherwise
...
private fun onStorageEvent(listener: () -> Unit) {
    js("window.addEventListener('storage', function () { listener(); })")
}
