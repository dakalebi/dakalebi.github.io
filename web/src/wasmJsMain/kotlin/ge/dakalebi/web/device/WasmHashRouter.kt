 ...
 *
 * Every route the app can be in is expressible as a URL, which is what makes a hard refresh
 * and a copied link land in the same place. The rules themselves — [Router.href] and
 * [Router.parse] — are shared; this only binds them to `window.location`.
 */
class WasmHashRouter : Router {
    ...
    /**
     * Built from `pathname` rather than assumed to be at the site root, so this stays correct
     * under the `/preview` sub-path.
     */
